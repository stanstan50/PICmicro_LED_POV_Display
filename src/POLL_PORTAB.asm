...
__CONFIG _INTOSC_OSC_NOCLKOUT & _WDTE_OFF & _MCLRE_OFF & _PWRTE_OFF & _BOREN_OFF & _LVP_OFF & _CPD_ON & _CP_OFF
...
INTWBUFFER
DELAY4_BUFFER
RSTATES	; bit0 - Previous State of RB0
    endc
...
LoopMessage
    ...
    ; Check if sensor state is equal to previous state (use TEMP)
    ; TEMP.0 <- PORTB.0 & RSTATES.0
    BSF	    TEMP, 0
    BTFSC   PORTB, 0
    BTFSS   RSTATES, 0
    BCF	    TEMP, 0
    ...
    BTFSC   TEMP, 0
    goto    EndPoll	; EndPoll if equal to previoius state
    
    ; if RB0 == 0 and is not equal to previous state (FALLING EDGE)
    call    SetDelay
